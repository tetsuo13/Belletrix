...
$(document).ready(function () {
    ...
    //#endregion "Language"
    //#region "Column"
    var colCreatedCellFunc = function (cell, cellData, rowData, rowIndex, colIndex) {
    };
    var colDataObject = {
        _: "phone",
        filter: "phone_filter",
    ...
    var colDataFunc = function (row, type, set, meta) {
        meta.col;
        meta.row;
        meta.settings;
    };
    ...
    var colRenderFunc = function (data, type, row, meta) {
        meta.col;
        meta.row;
        meta.settings;
    };
    ...
    //#endregion "ColumnDef"
    //#region "Callbacks"
    var createRowFunc = function (row, data, dataIndex) {
    };
    var drawCallbackFunc = function (settings) {
    };
    var footerCallbackFunc = function (tfoot, data, start, end, display) {
    };
    var formatNumberFunc = function (toForm) {
    };
    var headerCallbackFunc = function (thead, data, start, end, display) {
    };
    var infoCallbackFunc = function (settings, start, end, total, pre) {
    };
    var initCallbackFunc = function (settings, json) {
    };
    var preDrawFunc = function (settings) {
    };
    var rowCallbackFunc = function (row, data) {
    };
    var stateLoadCallbackFunc = function (settings) {
    };
    var stateLoadedCallbackFunc = function (settings, data) {
    };
    var stateSaveCallbackFunc = function (settings, data) {
    };
    var stateSaveParamsFunc = function (settings, data) {
    };
    //#endregion "Callbacks
    //#region "Ajax"
    var ajaxFunc = function (data, callback, settings) {
    };
    var ajaxDataFunc = function (data) {
        return data;
    };
    ...
    var select = $('<select />').appendTo('body').on('change', function () {
        dt.column(0).search($(this).val()).draw();
    });
    // Get the search data for the first column and add to the select list
    var data = dt.cells('', 0).cache('search').sort().unique().each(function (d) {
        select.append($('<option value="' + d + '">' + d + '</option>'));
    });
    var cells_data = cells.data();
    var data = dt.cells(".info").data();
    console.log(data);
    var cells_indexes = cells.indexes();
    var columns_d = dt.cells(':contains("21")').indexes().pluck('column').sort().unique();
    ...
    var cells_d = dt.cells(":contains('Not shipped')").nodes();
    ...
    $('#example tbody').on('click', 'td', function () {
        var rowIdx = dt.cell(this).index().row;
    ...
    var cell_n = dt.cell("#importantCell").node();
    ...
    dt.columns('.select-filter').eq(0).each(function (colIdx) {
        // Create the select list and search operation
        var select = $('<select />').appendTo(dt.column(colIdx).footer()).on('change', function () {
            dt.column(colIdx).search($(this).val()).draw();
        });
        // Get the search data for the first column and add to the select list
        dt.column(colIdx).cache('search').sort().unique().each(function (d) {
            select.append($('<option value="' + d + '">' + d + '</option>'));
        });
    });
    var columns_data = columns.data();
    $('#listData').html(dt.columns(0).data().eq(0).sort().unique().join('<br>'));
    //var idx = dt
    //    .columns('.check')
    //    .data()
    ...
    // Create the select list and search operation
    var select = $('<select />').appendTo(dt.column(0).footer()).on('change', function () {
        dt.column(0).search($(this).val()).draw();
    });
    // Get the search data for the first column and add to the select list
    dt.column(0).cache('search').sort().unique().each(function (d) {
        select.append($('<option value="' + d + '">' + d + '</option>'));
    });
    var column_data = column.data();
    alert('Column 4 sum: ' + dt.column(4).data().reduce(function (a, b) {
        return a + b;
    }));
    ...
    $('#column3_search').on('keyup', function () {
        dt.columns(3).search(this.value).draw();
    });
    dt.columns('.select-filter').eq(0).each(function (colIdx) {
        // Create the select list and search operation
        var select = $('<select />').appendTo(dt.column(colIdx).footer()).on('change', function () {
    ...
    alert('Column index 0 is ' + (dt.column(0).visible() === true ? 'visible' : 'not visible'));
    for (var i = 0; i < 4; i++) {
    ...
    dt.rows().eq(0).each(function (rowIdx) {
        dt.row(rowIdx).child($('<tr>' + '<td>' + rowIdx + '.1</td>' + '<td>' + rowIdx + '.2</td>' + '<td>' + rowIdx + '.3</td>' + '<td>' + rowIdx + '.4</td>' + '</tr>')).show();
    });
    $('#example tbody').on('click', 'td.details-control', function () {
        var tr = $(this).parents('tr');
        var row = dt.row(tr);
...
//# sourceMappingURL=jquery.dataTables-tests.js.map